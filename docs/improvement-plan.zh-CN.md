...
> - 想找**本轮修了什么** → §10.1 / §10.2 / §10.5 与 §10.6 / §10.7；
...
| **D9** | **基础设置（风格设置）的文本输入框改为「失焦后保存」**：LOGO 文字 / 壁纸地址 / 最大宽度 / 页脚 HTML 不再逐键触发防抖保存，焦点离开输入框（点击输入框外）后统一提交；开关 / 滑块 / 下拉 / 颜色选择器与搜索引擎列表保持「变化即保存」；手动「保存」按钮与进行中的自动保存合并，不重复提交 | 打字过程不再把中间状态写库；「编辑中」状态由 `document.activeElement` 实时推导（不依赖 focus/blur 事件配对，事件计数失衡不会卡死保存），见 §10.7 |

> D8 是计划结项后按用户需求追加的一项（原计划未包含），已实现并验证，详见 §9.0。
>
> D9 也是计划结项后按用户需求追加的一项（原计划未包含），已实现并验证，详见 §10.7。
...
| 分组必填误报（P0） | 分组规则改为显式 `validator`：async-validator 在规则含 `required` 以外键（如 `trigger`）时会退回 `string` 校验器，数字 id 被判类型错误 → 明明选好分组也报「必填项」，**新增与修改项目都无法保存**。新规则接受 >0 的数字 / 数字字符串，0 与缺失视为未选择（与后端 1404 对齐） | `frontend/src/views/home/components/EditItem/index.vue`；端到端新增与修改均保存成功 |
| 分组链路重构（方案 C + D1a/D2a/D3b/D4a/D5a/D6a/D7a） | 打开弹窗先用父组件已加载的分组列表填满下拉（打开即可显示组名与切换），后台再 `getList` 刷新；刷新失败有列表则静默、无列表则禁用 + 灰字；值必须匹配可用列表否则清空（删除「落到第一个分组」旧兜底）；保存前复查分组：失效则清空 + 展开更多选项 + 字段级提示并中止，列表不可用则阻断保存 | `EditItem`（props `itemGroups`、状态机、`seedOptionsFromInherited` / `loadGroupOptions` / `enforceGroupInvariant`）、`index.vue`（`handleAddItem(group)`、`:item-groups="items"`）、新增 i18n 键 `iconItem.getGroupFailRetry`，详见 §10.6 |
| 卡片背景色只影响弹窗预览 | 重建 `model` 时对 `icon` 浅拷贝：弹窗内改色只改预览，首页卡片要等保存成功后才变（取消 / 失败不变）。修复前 `model.icon` 与首页项目共享同一对象，颜色选择器即时写回且取消不回滚 | 同上；端到端：弹窗改蓝 → 首页仍黑 → 取消后仍黑 → 改绿保存后首页变绿 |
...
### 10.6 本轮第三批（分组链路重构 + 分组必填误报）

> 触发：用户反馈「新增项目 → 更多选项 → 分组一直显示必填项」。排查确认这不是显示问题：该规则让**新增与修改项目都无法保存**（表单永远校验不过）。本轮同时按用户逐条确认的决策把「分组」这条链路整体重做。

| 决策 | 结论与落地 | 验证 |
|------|-----------|------|
| **分组必填误报（P0）** | async-validator 的隐蔽行为：规则里只要 `required` 之外还有别的键（这里是 `trigger`），它就不再走「仅 required」分支，而是退回 `string` 校验器 —— 分组 id 是数字，被判成类型错误并按规则自身的 `message` 报「必填项」。改为显式 `validator`：>0 的数字（兼容数字字符串）通过，0 / 缺失视为「未选择」（与后端 1404 对齐） | 端到端：新增、修改均能保存；分组字段不再误报 |
| **① 方案 C：下拉改用父组件已加载的列表** | 弹窗打开时先用首页 `items` 填满下拉（**打开瞬间**即显示组名并可切换），同时后台 `getList` 刷新；刷新失败时：手里有列表 → 静默（D2a），完全无列表 → 禁用下拉 + 字段下方一行灰字（D1a） | 端到端：打开即正确、可切换；拦截 500 → 列表仍可用且无额外提示；无继承列表 + 500 → 禁用 + 灰字 |
| **③-2 分组值不变量（D4a / D5a）** | 只要有可用列表，值就必须匹配其中一项，否则清空；折叠区关闭时静默、已展开（或保存流程要求）时立即渲染字段级「必填项」；删除旧的「没有分组就落到第一个分组」兜底 | 端到端：软删分组后打开编辑弹窗 → 值被清空 + 红字当场出现 |
| **③-3 / ③-4 保存前复查（D3 / D3b）** | 点保存时重拉一次列表并校验（顺带刷新下拉）：值失效 → 清空 + 展开更多选项 + 字段级提示 + **中止保存**；列表不可用 → **阻断保存**并提示「分组信息获取失败，请稍后重试」（新增 i18n 键） | 端到端：复查失效 → 中止且未写入；拦截 500 → 阻断 + 新提示，弹窗保持、无新记录 |
| **③-1 无参新增清空上下文（D6a）** | `handleAddItem` 改为接收分组对象并**无条件赋值**：不带上下文 = 明确清空，不再沿用上一次用过的分组 | 端到端：无参调用打开 → 分组为空 → 保存时自动展开 + 字段级「必填项」 |
| **②-2 开关复位** | 每次打开：效果预览 = 开、画布透明 = 关；更多选项 = 自定义背景色（≠ `#2a2a2a6b`，**空值视为未设置**）或唯一标识非空 | 端到端：新增折叠；有唯一标识展开；有自定义背景色展开 |
| **小隐患：背景色只影响预览** | 重建 `model` 时对 `icon` 做一层浅拷贝。修复前 `model.icon` 与首页项目是**同一个对象**，而颜色选择器每次交互都会即时 emit（naive-ui 的 `doUpdateValue`），因此弹窗里改色会直接改到首页卡片，取消也不回滚 | 端到端：弹窗改蓝 → 首页仍黑；取消后仍黑；改绿并保存 → 首页变绿 |

**新增 i18n 键**：`iconItem.getGroupFailRetry`（zh/en 各一条；i18n 审计缺失 0 / 中英不齐 0 / 死文案 0）。
**请求数变化**：新增/编辑一次项目由 3 次变为 4 次（多出的一次是保存前复查，用户已确认）。
**已知取舍**：D3b 把「分组列表接口不可用」升级为「暂时无法新增/编辑项目」（阻断而不是猜测）；D1a 的显示层隔离（`groupSelectValue`）：完全没有可用列表时不再把裸 id 渲染到界面上，模型仍保留值 —— 列表一到，同一份值立刻重新渲染成组名（新增验证：无列表 + 500 时字段为空且禁用，去掉拦截后点保存即可把项目写进该分组）。

### 10.7 本轮第四批（基础设置保存时机：失焦后保存）

> 触发：用户提出「基础设置的文本输入框可否改为点击输入框外时退出输入再保存」。排查确认：全仓库只有基础设置（`Style/index.vue`）是「变化即保存」（文本输入逐键触发 1 秒防抖）；全局设置、个人信息、分组管理与搜索引擎编辑弹窗均为显式按钮/确认保存，不在改动范围。

| 决策 | 结论与落地 | 验证 |
|------|-----------|------|
| **文本输入框失焦后保存** | 4 个文本类输入框（LOGO 文字 `logoText`、壁纸地址 `backgroundImageSrc`、最大宽度 `maxWidth`、页脚 HTML `footerHtml`）加 `data-hold-save` 标记与 `@blur`；`scheduleSave()` 门口新增挂起分支：焦点落在任一标记输入框内（`document.activeElement.closest('[data-hold-save]')`）时只置 `dirtyWhileEditing`、不提交，由失焦检查点统一冲账 | 端到端：打字全程无 `userConfig/set` 请求；失焦后防抖窗口内提交并弹「配置已保存」 |
| **放弃事件计数、改用焦点实查** | 首版用 focus/blur 事件计数（focus +1 / blur −1）判断「编辑中」，`handleTextInputBlur` 漏写 −1 使计数只增不减，保存被**永久挂起**（症状：失焦与点击其他控件均不再弹「配置已保存」）。修复改为每个判断点直接读 `document.activeElement`：状态不靠历史事件累加，事件丢失/重复、组件卸载都不会再卡死保存 | 通过 Vue 组件实例读到 `textEditingCount` 卡死为根因；修复后全部场景复测通过 |
| **Tab 切换与卸载兜底** | blur 检查点延迟一拍（`setTimeout(0)`）：Tab 切换时旧输入框的 blur 先于新输入框的 focus 触发，延迟后读到稳定的最终焦点再决定是否冲账，避免中途误存；切换侧栏应用会卸载组件、最后一次 blur 可能被吞，`onBeforeUnmount` 里补交挂起的改动（防抖回调闭包在卸载后照常执行） | Tab 切换：中途无请求、离开后保存一次；输入后直接切换子应用：保存触发、切回后值保留 |
| **手动保存与自动保存合并** | 「保存」按钮改走 `saveNow()`：`isSaveing` 期间置 `savePending` 由自动保存链路补一次，而不是并发第二个请求；无保存进行中时行为不变（立即 `recordState()` + `uploadCloud()`） | 5 次连续操作恰好 5 个 `userConfig/set`，无重复提交 |

**改动范围**：仅 `frontend/src/components/apps/Style/index.vue`。开关 / 滑块 / 下拉 / 颜色选择器 / 搜索引擎列表的「变化即保存」路径未动（点击这些控件前文本框必然先失焦，挂起分支不会被触发）；增加卡片（EditItem）与全局设置、个人信息、分组管理等显式保存模块不受影响。

**端到端验证**（本地 `wrangler dev` + `vite dev`，Playwright 实测）：输入不保存 → 失焦保存并弹提示 → Tab 切换不误存 → 开关/滑块即时保存 → 手动保存无重复请求 → 切换子应用补交改动 → 切回后值保留。
...
| 本轮（基础设置失焦保存） | 基础设置（`Style/index.vue`）的 4 个文本输入框（LOGO 文字 / 壁纸地址 / 最大宽度 / 页脚 HTML）从「逐键防抖自动保存」改为「失焦后保存」（决策 D9、记录见 §10.7）：`data-hold-save` 标记 + `document.activeElement` 实时判定「编辑中」+ `@blur` 检查点（延迟一拍防 Tab 误存）+ `onBeforeUnmount` 卸载补交；手动「保存」改走 `saveNow()` 与自动保存合并；首版事件计数失衡导致保存永久挂起的缺陷由端到端实测定位并修复；开关 / 滑块 / 颜色选择器 / 搜索引擎列表的「变化即保存」与显式保存模块（全局设置 / 个人信息 / 分组管理 / 卡片编辑）不受影响。本地 `wrangler dev` + Playwright 端到端验证通过 |
