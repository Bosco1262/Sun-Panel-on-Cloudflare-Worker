...
| Group "required" false positive (P0) | the group rule became an explicit `validator`: async-validator falls back to the `string` validator whenever a rule carries any key besides `required` (here `trigger`), so the numeric group id failed a type check and reported the rule's own "required" message — **neither adding nor editing an item could be saved**. The new rule accepts a number > 0 (or a numeric string) and treats 0 / missing as "not selected", matching the backend's 1404 | `frontend/src/views/home/components/EditItem/index.vue`; end-to-end: both add and edit save successfully |
| Group field chain rebuilt (option C + D1a/D2a/D3b/D4a/D5a/D6a/D7a) | the dialog fills its dropdown from the group list the home page already loaded (a correct name and switching available the moment it opens) and refreshes with `getList` in the background; a failed refresh stays silent when a list is at hand and disables the select with an inline hint when there is none; the value must match an available list entry or it is cleared (the old "fall back to the first group" behaviour is gone); before saving the group is re-checked: an invalid value is cleared, reveals the collapse area and shows the field-level error, while an unreachable list blocks the save | `EditItem` (`itemGroups` prop, state machine, `seedOptionsFromInherited` / `loadGroupOptions` / `enforceGroupInvariant`), `index.vue` (`handleAddItem(group)`, `:item-groups="items"`), new i18n key `iconItem.getGroupFailRetry`, see §10.6 |
| Card background colour affects the preview only | the model rebuild now copies `icon` one level deep, so picking a colour inside the dialog only changes the preview and the home page card follows once the save succeeds (cancel / failure leave it untouched). Before the fix `model.icon` was the very object the home page renders and the colour picker emitted on every interaction (naive-ui's `doUpdateValue`), so a cancelled edit still recoloured the card | same files; end-to-end: blue inside the dialog → home page still black after cancelling → green after saving |
...
### 10.6 The third batch of this round (group field chain rebuild + the "required" false positive)

> Trigger: the user reported "adding an item → more options → the group field always shows 'required'". The investigation
> showed it was not a display problem: that rule made **both adding and editing an item impossible to save** (the form could
> never pass validation). This round also rebuilds the whole group field chain along the decisions the user confirmed one by one.

| Decision | Conclusion and outcome | Verification |
|----------|------------------------|--------------|
| **Group "required" false positive (P0)** | A subtle async-validator behaviour: as soon as a rule carries any key besides `required` (here `trigger`), it no longer takes the "required only" shortcut but falls back to the `string` validator — the group id is a number, so it failed a type check and reported the rule's own message, "required". The rule became an explicit `validator`: a number > 0 (a numeric string too) passes, 0 / missing counts as "not selected" (matching the backend's 1404) | End-to-end: adding and editing both save; the field no longer reports a false error |
| **① Option C: the dropdown uses the list the parent already loaded** | Opening the dialog fills the dropdown from the home page's `items` (correct name and switching available **immediately**), while `getList` refreshes it in the background. A failed refresh stays silent when a list is at hand (D2a) and disables the select with one inline grey line when there is none (D1a) | End-to-end: correct on open and switchable; a mocked 500 keeps the list usable with no extra message; no inherited list + 500 → disabled + inline hint |
| **③-2 Group invariant (D4a / D5a)** | With a usable list at hand the value must match one of its entries, otherwise it is cleared; clearing stays silent while the collapse area is closed and renders the field-level "required" immediately when it is open (or when the save flow asks for it). The old "fall back to the first group" behaviour is gone | End-to-end: after soft-deleting a group, opening the edit dialog clears the value and shows the error right away |
| **③-3 / ③-4 Pre-save re-check (D3 / D3b)** | Saving fetches the list once more (which also refreshes the dropdown): an invalid value is cleared, the collapse area is revealed and the field-level error is shown, then the save is **aborted**; an unreachable list **blocks** the save with "Failed to get group information, please try again later" (new i18n key) | End-to-end: an invalid value aborts without writing; a mocked 500 blocks with the new message, the dialog stays open and no row is written |
| **③-1 Clearing the context for a call without a group (D6a)** | `handleAddItem` now takes the group object and assigns **unconditionally**: no context means an explicit clear instead of silently reusing the group used last time | End-to-end: calling it without arguments opens with an empty group → saving reveals the collapse area and shows the field-level "required" |
| **②-2 Switch resets** | On every open: preview on, transparent canvas off, and the collapse area open only for a custom background colour (≠ `#2a2a2a6b`, **an empty value counts as "not set"**) or a non-empty unique identifier | End-to-end: collapsed for a new item; expanded for a unique identifier; expanded for a custom background colour |
| **Latent defect: background colour only affects the preview** | The model rebuild now copies `icon` one level deep. Before the fix `model.icon` was the **same object** the home page renders and every colour-picker interaction emits immediately (naive-ui's `doUpdateValue`), so recolouring inside the dialog changed the home page card and cancelling did not roll it back | End-to-end: blue inside the dialog → home page still black; still black after cancelling; green after saving |

**New i18n key**: `iconItem.getGroupFailRetry` (one entry per locale; the i18n audit reports 0 missing, 0 one-sided, 0 dead keys).
