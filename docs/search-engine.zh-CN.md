...
- 老版本把引擎存在 `module_config` 的 `module-deskModuleSearchBox` 里：这条兼容迁移已随死代码清理
  **一并移除**（见 [improvement-plan.zh-CN.md](./improvement-plan.zh-CN.md) §5.1，`module_config` 表也不再创建）。
...
本移植版是**单用户版本，没有公开模式**（见根 README 与 [deployment.zh-CN.md](./deployment.zh-CN.md)
...
（在仓库根目录执行；**完整脚本清单与命令见
[improvement-plan.zh-CN.md 附录 C](./improvement-plan.zh-CN.md#附录-c自检脚本与命令)**）。与本页相关的三个：
