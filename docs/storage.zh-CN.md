...
> 备份与恢复步骤见 [deployment.zh-CN.md](./deployment.zh-CN.md#备份与恢复)；结构变更约定见
> [improvement-plan.zh-CN.md](./improvement-plan.zh-CN.md) §2.2。

## 1. Cloudflare 侧资源

| 资源 | 绑定 | 名称 | 用途 | 备注 |
|------|------|------|------|------|
| D1 | `DB` | `sun-panel-on-cloudflare-worker-db` | 全部业务数据（见第 2 节） | wrangler ≥ 4.45 部署时自动创建 |
...
> （见 [improvement-plan.zh-CN.md](./improvement-plan.zh-CN.md) §2.3 / §2.4）。云端那个自动创建的 KV namespace
...
> **已移除**（不再创建，见 [improvement-plan.zh-CN.md](./improvement-plan.zh-CN.md) §5.1 与 §10.5）：`module_config`（旧搜索框配置，仅剩的迁移读取也已删除）、
...
> 代码不再访问它们，保留无害；若要彻底清理，请先备份再按 [improvement-plan.zh-CN.md 附录 A](./improvement-plan.zh-CN.md#附录-a0001_initsql-结构5.1-之后的现状) 的说明确认结构。
...
见 [deployment.zh-CN.md 的「备份与恢复」](./deployment.zh-CN.md#备份与恢复)：D1 用 `wrangler d1 export`，
...
| 项目 | 当前用量（实测） | 免费层额度 | 结论 |
|------|------------------|-----------|------|
| 库存储 | `sun-panel-on-cloudflare-worker-db` **77.8 kB**（6 张业务表 + 历史表） | 单库 **500 MB**、账户总量 5 GB | 余量 3 个数量级 |
| 账户库数量 | 4 个（sun-panel-on-cloudflare-worker-db / m365-copilot2api / inkstone-db / cloud-mail，合计约 1.3 MB） | **10 个/账户** | 余 6 个（改名过渡期会多一个旧库，删掉即恢复） |
...
本项目**已不再使用 KV**（登录限流已迁到 D1，见 [improvement-plan.zh-CN.md §2.3 / §2.4](./improvement-plan.zh-CN.md)）。
